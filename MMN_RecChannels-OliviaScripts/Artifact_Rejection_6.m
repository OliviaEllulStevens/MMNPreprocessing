...
%%%% Must alter the three excel spreadsheets below

%Load the Excel file with the list of channels to interpolate for each subject 
[ndata1, text1, alldata1] = xlsread([Current_File_Path filesep 'Interpolate_Channels_MMN']); %%%% must meet <25% rejected quality thresholds
...
%%%% Contains threshold, timewindow (-200-800ms by default), window size and window step
%%%% Within any 1000ms window size, is the peak to peak amplitude bigger than threshold? Shifts by 100ms each time (step) so big overlap
...
for i = 1:length(SUB)
    ...
    for j = 1:DimensionsOfFile1(1);
        if isequal(SUB{i},num2str(alldata1{j,1}));
           ...
           if ~isequal(badchans,'none') | ~isempty(badchans)
              ...
              % ---- Fix for missing chanlocs.urchan (needed by ERPLAB interpolate) ----
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4, 'setname', [SUB{i} '_MMN_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch_interp_ar'], 'savenew', [Subject_Path SUB{i} '_MMN_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch_interp_ar.set'], 'gui', 'off'); 

    % Changed last file save name to not mess up next script.
