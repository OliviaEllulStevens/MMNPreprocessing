...
for i = 1:length(SUB)
    ...
    EEG = pop_runica(EEG,'extended',1,'chanind', [1:30]);
    ...
    pop_topoplot(EEG, 0, [1:30],[SUB{i} '_MMN_ds_reref_ucbip_hpfilt_ica_prep2_weighted'], [6 6] ,0,'electrodes','on');
    save2pdf([Subject_Path 'graphs' filesep SUB{i} '_MMN_ICA_Weights.pdf']);
    close all
    
    %%%% Can look through ICA components via EEGLAB, can auto generate labels to help with exclusions. 
