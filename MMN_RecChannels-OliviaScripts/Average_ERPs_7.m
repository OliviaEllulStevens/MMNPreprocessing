...
%%%% Averages non-rejected epochs to produce ERP/bin, applies LPF for peak visualisation.
...
for i = 1:length(SUB)
    ...
    ERP = pop_filterp( ERP,  1:32 , 'Cutoff',  20, 'Design', 'butter', 'Filter', 'lowpass', 'Order',  8 );
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_MMN_erp_ar_lpfilt'], 'filename', [Subject_Path SUB{i} '_MMN_erp_ar_lpfilt.erp']);
    %%%% Allows low freq components yo pass through while removing high freq components -- reducing high freq noise (e.g., muscle movements). 
    %%%% Non-causal = using info from the past and future, relative to each time point, producing ZERO PHASE DISTORTION (no ERP peak shift). 
    %%%% 20Hz cut off = freqs lower than 20Hz are unchanged, freq at 20Hz reduced to ~ 70% amplitude freq above 20Hz are progressively attenuated.
    %%%% 48dB/oct roll-off = how quickly the filtr suppresses freq above cutoff - everytime freq doubles beyond the cutoff, the filter
    %%%% reduces it by 48dB, allows for muscle noise above 20Hz to be aggressively suppressed
    ...
    % Calculate the total percentage of trials rejected across all trial types (bins 1–8)
...
%Create difference waveforms = ERPDeviant - ERPStandard 
...
for i = 1:length(SUB)
    ...
    ERP = pop_binoperator( ERP, [Current_File_Path filesep 'MMN_Diff_Wave.txt']); %%%% Added bins 9, 10, 11 (diff between deviant-standard means).
