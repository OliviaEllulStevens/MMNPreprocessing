...
for i = 1:length(SUB)
    ...
    %%%% Changed to load the raw continuous EEG data file in .bdf 
     % "Pop" is a wrapper function, calling biosig to read in data, "EEG" is a MATLAB structure containing everything about a dataset
    EEG = pop_biosig([Subject_Path SUB{i} '_MMN.bdf']);

    [ALLEEG, EEG, CURRENTSET] = pop_newset([], EEG, 0, ...
        'setname', [SUB{i} '_MMN'], ...
        'gui', 'off');
